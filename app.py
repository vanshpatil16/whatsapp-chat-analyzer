...
# Chat file selection using selectbox
chat_option = st.selectbox("Choose a sample chat file:", [ "_chat.txt", "sample_chat.txt","None"])

# Load the selected chat file
if chat_option == "_chat.txt":
    data= load_chat_file("_chat.txt")
    st.success("Loaded _chat.txt!")
elif chat_option == "sample_chat.txt":
    data = load_chat_file("sample_chat.txt")
    st.success("Loaded sample_chat.txt!")
...
if st.sidebar.button("Most avaliable users"):
...
if st.sidebar.button("Rate of messages of {}".format(selected_user)):
    ...
    plt.figure(figsize=(10,10))# Rotated x-axis labels for better visibili
    st.pyplot(fig1)
